...
export default function AccountPanel({
    ...
    const fetchAccountData = async () => {
        ...
        try {
            ...
            // obtener usuario
            const uUrl = `${SUPABASE_URL}/rest/v1/usuario?correo=eq.${encodeURIComponent(email)}&select=correo,nombre_usuario`;
            ...
            // obtener cuenta
            const cUrl = `${SUPABASE_URL}/rest/v1/cuenta?correo_usuario=eq.${encodeURIComponent(email)}&select=correo_cuenta,nombre_cuenta,location,workplace`;
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            // PATCH usuario -> nombre_usuario
            if (usuario) {
                const payload = { nombre_usuario: usuario.nombre_usuario ?? null };
                const uUrl = `${SUPABASE_URL}/rest/v1/usuario?correo=eq.${encodeURIComponent(email)}`;
                const uRes = await fetch(uUrl, {
                    method: "PATCH",
                    headers: accessToken ? headersAuth : headersBase,
                ...
                if (!uRes.ok) {
                    const txt = await uRes.text().catch(() => null);
                    console.warn("update usuario failed", uRes.status, txt, {
                        uUrl,
                        payload,
                    });
                    // no throw: tolerante
            ...
            // PATCH cuenta -> nombre_cuenta, location, workplace
            if (cuenta) {
                const payload = {
                    nombre_cuenta: cuenta.nombre_cuenta ?? null,
                    location: cuenta.location ?? null,
                    workplace: cuenta.workplace ?? null,
                };
                const cUrl = `${SUPABASE_URL}/rest/v1/cuenta?correo_usuario=eq.${encodeURIComponent(email)}`;
                const cRes = await fetch(cUrl, {
                    method: "PATCH",
                    headers: accessToken ? headersAuth : headersBase,
                    body: JSON.stringify(payload),
                ...
                if (!cRes.ok) {
                    const txt = await cRes.text().catch(() => null);
                    console.warn("update cuenta failed", cRes.status, txt, {
                        cUrl,
                        payload,
                    });
                    // tolerante
    ...
    const handleOpenChangePassword = () => {
        ...
        // fallback: open the forgot-password flow if no callback was provided
        window.alert(
            "Abrir interfaz de cambio de contraseña (fallback): usa la funcionalidad integrada.",
        );
        // you can also do: window.location.href = '/#/forgot' depending on your routing
    };
                    ...
                    <div style={styles.grid}>
                        ...
                        <div>
                            ...
                            <div style={{ marginTop: 12, fontSize: 13, color: "#6b7280" }}>
                                ...
                                <div>
                                    Account created:{" "}
                                    <strong>{/* opcional: fecha si disponible */}</strong>
                                </div>
                            </div>
                        </div>
                    </div>
